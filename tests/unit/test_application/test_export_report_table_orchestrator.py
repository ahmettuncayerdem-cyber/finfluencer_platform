"""Tests for ExportReportTableOrchestrator (BACKLOG.md T-026).

Exercises Application -> Domain -> the real T-026 `MasterTableExportAdapter` -> real parquet
fixtures, end to end -- not mocks standing in for the whole chain. Only the three repositories
are fakes (Persistence remains abstract), same discipline as
`test_generate_report_orchestrator.py` (T-025).
"""
...
def test_execute_exports_a_report_citing_topics_and_sentiment_runs(tmp_path: Path) -> None:
    ...
    orchestrator = ExportReportTableOrchestrator(
...
def test_execute_rejects_report_with_no_citations(tmp_path: Path) -> None:
    ...
    orchestrator = ExportReportTableOrchestrator(
        ...
        table_exporter=MasterTableExportAdapter(base_root=tmp_path),
    )
...
def test_orchestrator_module_never_imports_pandas_or_infrastructure() -> None:
    import finfluencer.application.orchestrators.export_report_table as module
