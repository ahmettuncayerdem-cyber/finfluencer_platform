...
`Tenant`, `Project`, `Dataset`, `CollectionRun`. The other eleven (`User`, `TenantMembership`,
`ProjectMembership`, `VerticalTemplate`, `AnalysisType`, `AnalysisRun`, `InterpretationRecord`,
`Report`, `Export`, `Subscription`, `AuditLogEntry`) are deliberately not implemented anywhere in
this package -- see the TODO comments in `tenant.py`, `project.py`, `dataset.py`, and
`collection_run.py` for exactly where each one's ownership relationship will attach, cited by
