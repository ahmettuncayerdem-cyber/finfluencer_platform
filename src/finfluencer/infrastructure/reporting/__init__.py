"""Reporting Infrastructure adapters (BACKLOG.md T-025, T-026, T-027, EPIC-10).
...
- `ChartRendererAdapter` (EPIC-10) implements `IChartRenderer` via `matplotlib`, reusing
  `reporting.master_table.build_master_table` unmodified -- same reuse target as
  `MasterTableExportAdapter`, for the same reason (see that adapter's own docstring).
...
from finfluencer.infrastructure.reporting.chart_renderer_adapter import ChartRendererAdapter
