...
| ID | Status | Reason | Owner | Next Review Point |
|---|---|---|---|---|
| ARB-01 | **Reviewed 2026-08-01 — no architectural changes authorized** | `IAnalysisEngine` plugin-pattern review, authorized and performed 2026-08-01. 10 dimensions evaluated: 8 Accept, 2 Generalize (low priority, non-blocking). Central finding: `AnalysisOutcome.topic_count` naming (see TD-03) confirmed by 4 independent review passes; architecture itself requires no change. Two candidate follow-up tasks proposed, not authorized. | Claude (reviewer) / Operator (approved) | Before a third `AnalysisType` is added (re-evaluate TD-03/TD-04 candidates then). |
...
| TD-03 | Open, classified **Generalize** — confirmed by 4 independent passes (T-022 adapter code comment, EGF Contract Stability Review, T-023 Special Focus, ARB-01) | `AnalysisOutcome.topic_count` is topic-modeling-named, reused for sentiment's distinct-class count (T-022). Field predates a second `AnalysisType`; changing it now touches T-019's frozen contract. Not fixed by T-023 or ARB-01 (both verification/recommendation-only scope); candidate follow-up task proposed, not authorized. | Unassigned, awaiting operator scheduling | Before a third `AnalysisType` is added. |
| TD-04 | Open, low priority, classified **Generalize** (ARB-01) | Context Pack file-naming asymmetry: `CONTEXT_PACK.md` (unsuffixed, Topics) vs. `CONTEXT_PACK_SENTIMENT.md` (suffixed, Sentiment) — convention never formalized before a second adapter existed. Candidate follow-up task proposed, not authorized. | Unassigned, awaiting operator scheduling | Before a third `AnalysisType` is added. |
