...
**Not started:** T-010 through T-032 (T-009 closed for Sprint 0 scope; full closure needs a future Persistence Layer task and API-layer routing).
...
**Critical path, restated with current position:** `[T-001 ✅] → T-002 ⏸ (env-blocked, no longer gates T-006) → [T-006 ✅] → [T-007 ✅] → [T-008 ✅ impl., review outstanding] → [T-009 ✅ Sprint 0 scope] → T-011 → T-013 → T-014 → …` — T-004 ✅, T-005 ✅, T-006 ✅, T-007 ✅ closed. T-008 implemented 2026-08-01 (cross-vendor review outstanding). T-009 implemented 2026-08-01 for Sprint 0 scope: `CreateProjectOrchestrator` + `IProjectRepository` interface, 108/108 tests passing (7 new), IG-001 clean; full closure against its original Definition of Done needs a future Persistence Layer implementation and API-layer routing, both explicitly out of this pass's scope by operator instruction. EPIC-01 (Shared Foundation) is now 5/5 implemented; EPIC-02 (Walking Skeleton) 1/6 for Sprint 0 scope. Next unblocked task: **T-010** (Infrastructure adapter wrapping `collect/`+`youtube.py`) or **T-011** (`StartCollectionRun` orchestrator) — T-010 depends on T-006/T-007 (both closed); T-011 depends on T-008/T-010.
