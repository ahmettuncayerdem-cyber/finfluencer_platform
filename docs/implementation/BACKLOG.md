...
**Epic progress:** EPIC-00 (Unfreeze the Repository) 1/3 tasks closed, 2 blocked-on-environment, no longer gating EPIC-01 (see Dependency Ruling below). EPIC-01 (Shared Foundation) 5/5 tasks closed (T-004, T-005, T-006, T-007, T-008 implementation — T-008's cross-vendor review outstanding, see its own entry). EPIC-02 through EPIC-07: not started. EPIC-08 (Housekeeping, non-blocking): 1 task opened, not started.

**Sprint progress (Sprint 0 = EPIC-00 through EPIC-02):** T-001, T-004, T-005, T-006, T-007, T-008 (implementation) of 14 Sprint-0 tasks closed. Sprint 0 exit gate (T-014, Walking Skeleton deployed) remains untouched.
...
**Not started:** T-009 through T-032.
...
**Critical path, restated with current position:** `[T-001 ✅] → T-002 ⏸ (env-blocked, no longer gates T-006) → [T-006 ✅] → [T-007 ✅] → [T-008 ✅ impl., review outstanding] → T-009 → T-011 → T-013 → T-014 → …` — T-004 ✅, T-005 ✅, T-006 ✅, T-007 ✅ closed. T-008 implemented 2026-08-01: three DTO pairs for `CreateProject`/`StartCollectionRun`/`GetCollectionRun`, 101/101 tests passing (26 new), IG-001 clean; its own Definition of Done requires a fresh-context/cross-vendor architecture review (Playbook Part B.1) not yet performed, tracked the same way T-007's was. EPIC-01 (Shared Foundation) is now 5/5 implemented. Next unblocked task: **T-009**, implement `CreateProject` command (EPIC-02, Walking Skeleton).
...
**Naming reconciliation, 2026-08-01:** this task's own Purpose line names the third operation "`GetCollectionRunStatus`," but §11.2's actual, binding Collection Service query is `GetCollectionRun` (line 684) — no `GetCollectionRunStatus` query exists in the approved contract. Implemented as `GetCollectionRun`, whose response includes `status` among the CollectionRun's wire-visible fields; flagged explicitly per this task's own "matches §11.2... for this slice" acceptance criterion rather than silently substituted. Documented in `presentation/dto/collection.py`'s module docstring.
